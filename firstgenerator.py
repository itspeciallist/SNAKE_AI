...
UPDATE_INTERVAL_MS = 5000  # Not used here but could be for UI update intervals
...
class NeuralNet:
    ...
    def mutate(self):
        def mutate_matrix(m):
            mutation_mask = np.random.rand(*m.shape) < MUTATION_RATE
            mutations = np.random.randn(*m.shape) * MUTATION_STRENGTH
            m += mutation_mask * mutations
...
def save_network(net, filename="best_snake_ai.pkl"):
...
def load_network(filename="best_snake_ai.pkl"):
    net = NeuralNet()
        ...
        while self.running:
            for _ in range(GENERATIONS_PER_UPDATE):
                ...
                if gen_best > self.best_score:
                    ...
                    save_network(self.best_net)  # Save best net to file
            print(f"Trainer best score: {self.best_score:.2f}")
...
def main():
    root = tk.Tk()
    root.title("Snake AI continuous learning")

    gui = None

    def update_best_net(new_net):
        nonlocal gui
        if gui is None:
            gui = SnakeGUI(root, new_net)
        else:
            gui.net = new_net
            gui.game.net = new_net

    trainer = TrainerThread(update_best_net)
    trainer.daemon = True
    trainer.start()
